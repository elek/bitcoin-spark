...
object BlockCount extends ConfigurableApplication[DefaultJobConfiguration] {
  ...
  def defaultConfiguration() = {
    new DefaultJobConfiguration(outputDir = "/result/block-count")
  }

  def run(session: SparkSession, configuration: DefaultJobConfiguration): Unit = {
    import session.sqlContext.implicits._
    val sum = (a: Int, b: Int) => a + b
    val reduce: Int = HBaseLoader.blocks(session.sqlContext).toDF().map(a => 1).reduce(sum)
